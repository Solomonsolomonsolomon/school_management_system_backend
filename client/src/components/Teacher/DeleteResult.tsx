...
const DeleteResult: React.FC<{ setOpenDeleteResultModal: any }> = ({
  setOpenDeleteResultModal,
}) => {
    ...
    (async () => {
      try {
        ...
        let res = await axios.delete(
          `${ResultUrl}/delete/${details.studentId}`,
          {
    ...
    <div className="">
      ...
      <section className="flex justify-center items-center place-items-center relative border ">
        ...
        <form action="" className="grid p-5 gap-2" onSubmit={handleSubmit}>
          ...
          {/* <input
            type="text"
            placeholder="enter class Result to delete"
            name="className"
            required
            onChange={handleFormInput}
            className="bg-inherit border border-gray-500 p-3 rounded "
      ...
      <WarningComponent>
        Result is deleted for  Current Year
      </WarningComponent>
    </div>
  );
};
