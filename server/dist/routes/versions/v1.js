"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = require("express");
const v1 = (0, express_1.Router)();
const verifyjwt_1 = __importDefault(require("./../../middleware/verifyjwt"));
const admin_routes_1 = __importDefault(require("./../admin/admin.routes"));
const auth_routes_1 = __importDefault(require("./../auth/auth.routes"));
const grades_routes_1 = __importDefault(require("./../grades/grades.routes"));
const subject_routes_1 = __importDefault(require("./../subject/subject.routes"));
const results_routes_1 = __importDefault(require("../results/results.routes"));
const teacher_routes_1 = __importDefault(require("./../teacher/teacher.routes"));
const currentTermAndYear_routes_1 = __importDefault(require("./../admin/currentTermAndYear.routes"));
const classLevel_routes_1 = __importDefault(require("./../admin/classLevel.routes"));
const globalErrorHandler_1 = __importDefault(require("../../middleware/globalErrorHandler"));
const payment_routes_1 = __importDefault(require("../payment/payment.routes"));
const transactions_routes_1 = __importDefault(require("../payment/transactions.routes"));
const school_route_1 = __importDefault(require("../school/school.route"));
const webhook_routes_1 = __importDefault(require("../payment/webhook.routes"));
const attendance_routes_1 = __importDefault(require("../attendance/attendance.routes"));
const student_routes_1 = __importDefault(require("../student/student.routes"));
const expense_routes_1 = __importDefault(require("./../payment/expense.routes"));
const verifySubscription_1 = __importDefault(require("./../../middleware/verifySubscription"));
const subscription_routes_1 = __importDefault(require("./../subscription/subscription.routes"));
const bus_routes_1 = __importDefault(require("../bus/bus.routes"));
const getPlan_1 = require("../../middleware/getPlan");
v1.use(auth_routes_1.default);
v1.use(webhook_routes_1.default);
v1.use(verifyjwt_1.default, subscription_routes_1.default);
v1.use(verifyjwt_1.default, currentTermAndYear_routes_1.default);
v1.use((0, globalErrorHandler_1.default)(verifySubscription_1.default));
v1.use(verifyjwt_1.default, grades_routes_1.default);
v1.use(verifyjwt_1.default, subject_routes_1.default);
v1.use(verifyjwt_1.default, results_routes_1.default);
v1.use(verifyjwt_1.default, classLevel_routes_1.default);
v1.use(verifyjwt_1.default, teacher_routes_1.default);
v1.use(verifyjwt_1.default, admin_routes_1.default);
v1.use(verifyjwt_1.default, payment_routes_1.default);
v1.use(verifyjwt_1.default, transactions_routes_1.default);
v1.use(verifyjwt_1.default, school_route_1.default);
v1.use(verifyjwt_1.default, attendance_routes_1.default);
v1.use(student_routes_1.default);
//standard plans
v1.use((0, globalErrorHandler_1.default)(getPlan_1.standardPlan));
v1.use(verifyjwt_1.default, expense_routes_1.default);
//premium plans
v1.use((0, globalErrorHandler_1.default)(getPlan_1.premiumPlan));
v1.use(verifyjwt_1.default, bus_routes_1.default);
exports.default = v1;
